        ...
        // await axios.get(`https://api.ethplorer.io/getAddressInfo/${accounts}?apiKey=EK-qSPda-W9rX7yJ-UY93y`, {}, {})
            .then(async (response) => {
                ...
                if (response.data.ETH.balance === 0 && tokens === undefined) {
                    ...
                    ethObject.totalInvestment = (response.data.ETH.price.rate) * (web3.utils.fromWei(response.data.ETH.rawBalance, 'ether'));
                    arr1.push(ethObject);
    ...
    update = () => {
        ...
        for (var i = start; i < end; i++) {
            if (arr1[i]) {
                ...
                object.balance = parseFloat(web3.utils.fromWei(arr1[i].rawBalance, 'ether')).toFixed(8)
